...
const LandingPage: React.FC<LandingPageProps> = ({ onShowAuth }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!formData.abstract.trim()) {
      alert('Please provide an abstract');
      return;
    }
    if (!formData.country) {
    ...
    if (!formData.novelty.trim()) {
      alert('Please describe the innovation and novelty');
      return;
    }
    if (!formData.utility.trim()) {
      alert('Please describe the practical utility');
      return;
    }
    if (formData.teamMembers.some(member => !member.name.trim() || !member.role.trim())) {
      alert('Please complete all team member information');
      return;
    }
